...
async function handleGameMessagePost(
  ...
  try {
    ...
    if (message) {
      // Log the game control message to server console.
      // eslint-disable-next-line no-console
      console.log(`[game-control] message from UI: ${message}`);
    }
    res.statusCode = 200;
    res.setHeader("Content-Type", "application/json");
...
export async function handleControlUiHttpRequest(
  ...
  // Handle game-message POST endpoint for the Game Control inbox.
